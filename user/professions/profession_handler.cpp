    ...
    auto summary = [&] {
        auto content = ftxui::vbox({
            ftxui::hbox({ftxui::text(
                             L"What performer/team created this song?") |
                         ftxui::bold}) |
                color(ftxui::Color::Orange4),
            ftxui::hbox({ftxui::text(
                             L"In the case of a contractor, give only his name.") |
                         ftxui::bold}) |
                color(ftxui::Color::Orange3),
            ftxui::hbox({ftxui::text(L"Enter your answer below") | ftxui::bold}) |
...
bool GuessDoctorQuestion(User &user) {
    ...
    if (documents.empty()) {
        std::cerr << "No questions found" << std::endl;
        return false;
    }
    ...
    auto summary = [&] {
        auto content = ftxui::vbox({
            ftxui::hbox({ftxui::text(
                             L"Diagnose the following disease based on the symptoms:") |
                         ftxui::bold}) |
                color(ftxui::Color::Orange4),
            ftxui::hbox({ftxui::text(question) | ftxui::bold}) |
                color(ftxui::Color::Orange3),
            ftxui::hbox({ftxui::text(
                             L"Hint: The name of the disease is always one word.") |
                         ftxui::bold}) |
...
bool GuessInformaticQuestion(User &user) {
    ...
    auto question_to_user = ftxui::text(L"Which line of code is the problem in?");
    auto container = ftxui::vbox({
        ftxui::hbox({ftxui::text("Programming language: " + language) | ftxui::bold}) |
            color(ftxui::Color::YellowLight),
        ftxui::separator(),
        code | ftxui::border,
        ftxui::separator(),
        ftxui::hbox({ftxui::text("Error in the displayed code: ") | ftxui::bold}) |
...
bool GuessMathQuestion(User &user) {
    ...
    if (solution_value.type() == bsoncxx::type::k_int32) {
        ...
        std::cerr << "Unknown type of solution" << std::endl;
        return false;
    }
    ...
    auto container = ftxui::vbox({
        ftxui::hbox({ftxui::text("Task from the section: " + topic) | ftxui::bold}) | color(ftxui::Color::YellowLight),
        ftxui::separator(),
        whole_problem | ftxui::border,
        ftxui::separator(),
        ftxui::hbox({ftxui::text("Hint: ") | ftxui::bold}) | color(ftxui::Color::BlueLight),
...
bool DisplayPoliceProfession() {
    auto summary = [&] {
        auto content = ftxui::vbox({
            ftxui::hbox({ftxui::text(L"Let's test you officer") | ftxui::bold}) |
                color(ftxui::Color::YellowLight),
            ftxui::hbox({ftxui::text(L"Enter your badge number") | ftxui::bold}) |
