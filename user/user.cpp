...
void User::ChangePassword(const std::string &password) {
    ...
    std::string repeated_password =
        DisplayMessageAndCaptureStringInput("Password change", "Enter the new password again to confirm the change:");
    if (password != repeated_password) {
        PrintErrorMessage("The passwords given are not the same.", "Try again.");
        return;
    }

    std::string old_password =
        DisplayMessageAndCaptureStringInput("Password change", "Enter the old password so we can be 200% sure it's you:");
    if (old_password != _password_) {
        PrintErrorMessage("The password provided is not correct.", "Try again.");
        return;
    }
    ...
    PrintSuccessMessage("The password has been successfully changed.", "");
}
...
void User::ChangeUsername(const std::string &username) {
    ...
    if (cursor.begin() != cursor.end()) {
        PrintErrorMessage("The username is already taken.", "");
        return;
    }
    ...
    username_ = username;
    PrintSuccessMessage("The user name has been successfully changed.", "");
}
...
void User::ChangeEmail(const std::string &email) {
    ...
    mongocxx::cursor cursor = _collection_.find(filter_view);
    if (cursor.begin() != cursor.end()) {
        PrintErrorMessage("This email address is already taken.", "");
        return;
    }
    ...
    email_ = email;
    PrintSuccessMessage("The email address has been successfully changed.", "");
}
...
Admin *User::LoginAsAdmin() {
    ...
    std::string
        password = DisplayMessageAndCaptureStringInput("Login as administrator", "Enter the administrator's password: ");
    std::string password_hashed = HashString(password);

    if (password_hashed == admin_password_hashed) {
        PrintSuccessMessage("Logged in as administrator.",
                            "We welcome you warmly. Use your power prudently.");
        ...
        PrintErrorMessage("Login failed.", "An incorrect administrator password was entered.");
        return nullptr;
    }
}
...
void User::SetDiscount(double discount, const std::string &discount_type) {
    ...
    PrintSuccessMessage("The discount has been credited to your account",
                        "You can see it in your profile and start using it already!");
}
...
void User::SetPremiumCard(User &user, const std::string &card) {
    ...
    PrintSuccessMessage("The card has been assigned to the account",
                        "You can see it in your profile and start using it already!");
}
...
void User::SetBlik(const std::string &payment_method) {
    ...
    std::string repeated_password = DisplayMessageAndCaptureStringInput("Change of payment method",
    ...
    PrintSuccessMessage("The payment method has been successfully changed.", "");
}
...
void User::SetVisa(const std::string &card_number, const std::string &card_cvv) {
    ...
    if (cursor_card_cvv.begin() != cursor_card_cvv.end()) {
        PrintErrorMessage("The card already exists in the database.", "");
        return;
    }
...
void User::AddTicketToUser(const std::vector<int> &seats, const FlightConnection &flight_connection) {
    ...
    if (seats.size() == 1) {
        PrintSuccessMessage("The ticket was successfully purchased.", "You can see it in the 'My Tickets' tab.");
    } else {
        PrintSuccessMessage("The tickets have been successfully purchased.", "You can see them in the 'My Tickets' tab.");
    }
}
...
void User::LuggageCheckin(int flight_number) {
    ...
    if (user_flights[flight_index]["luggageCheckin"].get_bool().value) {
        PrintErrorMessage("This flight has already been checked in.", "Choose another flight.");
        return;
    }
    ...
    PrintSuccessMessage("The luggage was checked in successfully!", "Have a good flight!");
}
