...
std::string displayDiscountInfo() {
    auto summary = [&] {
        ...
        auto content = ftxui::vbox({
                                           ...
                                           ftxui::hbox({ftxui::text(L"Każdej uldze przysługują konkretne zniżki.") | ftxui::bold}) | color(ftxui::Color::Orange4),
                                           ...
                                           ftxui::separator(),
                                           ftxui::hbox({ftxui::text(L"Wpisz numer ulgi, o którą chciałbyś się starać:")  | ftxui::bold}) | color(ftxui::Color::YellowLight),
                                   });
...
bool validDiscount(std::string discountChoice) {
    ...
    auto createDiscountScreen = [&] {
        auto summary = ftxui::vbox({
                                           ftxui::hbox({ftxui::paragraphAlignCenter("WERYFIKACJA ZNIŻKI")}) | color(ftxui::Color::GrayDark),
                                           ftxui::separator(),
                                           ftxui::hbox({
                                                               ftxui::text(discountText) | ftxui::bold | color(ftxui::Color::Green)}),
                                   });
