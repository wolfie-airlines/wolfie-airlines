...
void printSpecificItem(Item& item) {
    ...
    if (!hints.empty()) {
        ...
        elements.push_back(ftxui::hbox({ftxui::text("Ważne informacje: ") | ftxui::bold | ftxui::color(ftxui::Color::DarkSeaGreen2),
                                        hintBox}));
    }
    ...
    if (!profession.empty()) {
        professionElement = ftxui::hbox({ftxui::text("Dostępny w każdej ilości dla zawodu: ") | ftxui::bold | ftxui::color(ftxui::Color::Gold1),
    ...
    auto createScreen = [&] {
        auto summary = ftxui::vbox({
            ...
            ftxui::separator(),
            ftxui::hbox({ftxui::text("Przedmiot: ") | ftxui::bold | color(ftxui::Color::SkyBlue1),
                         ftxui::text(item.getItemName()) | color(ftxui::Color::White)}),
            ftxui::separator(),
            ftxui::hbox({ftxui::text("Opis przedmiotu: ") | ftxui::bold | color(ftxui::Color::DarkSeaGreen2),
                         ftxui::paragraph(description) | color(ftxui::Color::White)}),
            ftxui::separator(),
            ftxui::hbox({ftxui::text("Zabroniony w transporcie lotniczym: ") | ftxui::bold | color(ftxui::Color::RedLight),
                         ftxui::text(item.isForbidden() ? "TAK" : "NIE") | color(
                                                                               item.isForbidden() ? ftxui::Color::RedLight : ftxui::Color::GreenLight)}),
            ftxui::separator(),
            ftxui::hbox({ftxui::text("Przewóz w bagażu rejestrowanym: ") | ftxui::bold | color(ftxui::Color::Aquamarine1),
                         ftxui::text(item.isRegisteredLuggage() ? "✅" : "❌")}),
            ftxui::separator(),
            ftxui::hbox({ftxui::text("Przewóz w bagażu podręcznym: ") | ftxui::bold | color(ftxui::Color::MediumOrchid3),
                         ftxui::text(item.isHandLuggage() ? "✅" : "❌")}),
            ftxui::separator(),
            ftxui::hbox({ftxui::text("Wymagana zgoda pilota: ") | ftxui::bold | color(ftxui::Color::LightSkyBlue1),
                         ftxui::text(item.isPilotAllowance() ? "✅" : "❌")}),
            ftxui::separator(),
            ftxui::hbox({ftxui::text("Maksymalna ilość (sztuk): ") | ftxui::bold | color(ftxui::Color::LightCoral),
                         ftxui::text(std::to_string((int)item.getMaxCount())) | color(ftxui::Color::Salmon1)}),
            ftxui::separator(),
            ftxui::hbox({ftxui::text("Waga (jednej sztuki przedmiotu, wyznaczana na oko): ") | ftxui::bold | color(ftxui::Color::SandyBrown),
    ...
    while (true) {
        ...
        auto summary = ftxui::vbox({
            ...
            ftxui::vbox({
                ...
                ftxui::hbox({ftxui::text("quit. \U0001f51a  Wyjdź z listy") | ftxui::color(ftxui::Color::RedLight) | ftxui::bold}),
                ftxui::separator(),
                ftxui::hbox({
